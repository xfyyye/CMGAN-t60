"""
KAN-head 多任务模型的 T60 测试入口 (test1-test4)。

与单任务 test.py 唯一区别:
  - 加载 TSCNet_KAN_MultiTask{_2TSCB} 而不是 TSCNet_T60Estimator{_2TSCB}
  - forward 返回三元组 (est_real, est_imag, t60_pred)，只取 t60_pred 评估
  - 这里只评估 T60 性能，不评估去噪 (本实验聚焦 T60)
"""
...
def run_tests(args):
    ...
    model.eval()
    print(f'T60 head 类型: {args.t60_head_type} (multi-task forward)')
    ...
    print('四测试集平均')
    print(f'{"=" * 60}')
    avg = {k: float(np.mean([summary[t][k] for t in ['test1', 'test2', 'test3', 'test4']]))
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='CMGAN KAN multitask T60 测试')
    ...
    parser.add_argument('--n_tscb', type=int, default=config_defaults.get('n_tscb', 2), choices=[2, 4])
