#!/usr/bin/env bash
# Usage: ./train_multitask.sh -c configs/t60_multitask/kan_mse_w_eq.yaml -g 0,1
#
# Same shape as train.sh but invokes train_multitask.py (KAN head + denoise decoder
# + gradient angle probe). Dataset path is read from the YAML's data.dataset_root;
# pass -d to override at runtime.

set -euo pipefail

CONFIG="${CONFIG:-configs/t60_multitask/kan_mse_w_eq.yaml}"
...
usage() {
    echo "Usage: $0 [-c CONFIG] [-d DATASET_PATH] [-g GPUS] [--conda-env ENV] [--no-conda] [-- extra train_multitask.py args]"
    echo "  -c CONFIG       YAML config path. Default: configs/t60_multitask/kan_mse_w_eq.yaml"
...
cmd=("$PYTHON" train_multitask.py --config "$CONFIG" --train_only)
if [[ -n "$DATASET_ROOT_OVERRIDE" ]]; then
    cmd+=(--dataset_root "$DATASET_ROOT_OVERRIDE")
fi
cmd+=("${EXTRA_ARGS[@]}")

echo "Train (multitask + grad-probe): config=$CONFIG gpus=$GPUS"
if [[ -n "$DATASET_ROOT_OVERRIDE" ]]; then
