#!/usr/bin/env bash
# EMA归一化 + 权重组合: alpha=1.0 (denoise) / beta=0.05 (T60) —— 激进去噪主导（β 扫描点）
# 镜像 scripts/multitask_kan_norm_w_denoise.sh，默认 GPU 2/3（GPU 0/1 NVLink 异常）。
#
# Usage:
#   # 前台跑（默认 GPU 2,3）
#   ./scripts/multitask_kan_norm_w_denoise_b005.sh
#   # 后台跑（推荐长时训练）
#   setsid nohup bash scripts/multitask_kan_norm_w_denoise_b005.sh \
#       > runs/kan_multitask_norm_w_denoise_b005/nohup.log 2>&1 &
...
exec "${SCRIPT_DIR}/../train_multitask.sh" \
    -c "${SCRIPT_DIR}/../configs/t60_multitask/kan_mse_norm_w_denoise_b005.yaml" \
    "$@"
