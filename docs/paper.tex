...
To address these challenges, we propose a single-task $T_{60}$ estimation
framework built around two key design choices. First, we employ a
dual-path time--frequency Conformer encoder that applies self-attention
along the time and frequency axes in an alternating fashion, capturing
the long temporal context and frequency-dependent spectral coupling that
characterise late reverberation. Second, we introduce a lightweight
Fourier-basis Kolmogorov--Arnold regression head whose learnable
univariate edge functions replace the fixed scalar non-linearities of a
conventional MLP, supplying a strong inductive bias toward the smooth
low-dimensional mapping from pooled bottleneck statistics to $T_{60}$.
...
\begin{enumerate}[(1)]
  \item We design a dual-path time--frequency Conformer encoder for
        single-task blind $T_{60}$ estimation, where a three-channel
        complex STFT representation is projected by a dense convolutional
        front-end and refined by two stacked Conformer blocks that
        alternate self-attention along the time and frequency axes.
        This is the first work to introduce dual-path time--frequency
        Conformer blocks into the blind $T_{60}$ regression task.
  \item We design a lightweight Fourier-KAN regression head that maps
        multi-statistic pooled bottleneck features to $T_{60}$ through
        a two-layer Fourier-KAN block with a hierarchical frequency
        design. Experimental results confirm that the proposed head
        consistently outperforms a conventional MLP head of comparable
        size.
  \item We carry out a systematic evaluation on four noisy reverberant
        ...
        only approximately $1.35$\,M parameters, attains an average
        RMSE of $111.2$\,ms, a MAE of $72.4$\,ms and a Pearson
        correlation of $0.948$, and ablation studies confirm that each
...
formalises the acoustic signal model and the relation between the room
impulse response and $T_{60}$. Section~\ref{sec:method}
...
$\hat{T}_{60}$ directly, as proposed in
...
network. As illustrated in Fig.~\ref{fig:overall}, the
...
\end{equation}
Power-law magnitude compression
$|\mathbf{Y}|^{p}$, commonly used in speech enhancement, is not applied
here because $T_{60}$ depends on the decay shape of the envelope rather
than on its instantaneous loudness; the first convolutional layer can
learn a suitable normalisation from the data.
...
output linear. The objective is kept \emph{single-task}
without an auxiliary denoising loss for three reasons. First, the
dataset used in this work already provides
