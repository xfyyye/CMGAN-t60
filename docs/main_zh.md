...
| | Proposed | **132.2** | **93.2** | **65.5** | **70.1** | **81.8** | **90.1** |
...
| | Proposed | **144.7** | **111.0** | **82.1** | **68.5** | **71.6** | **95.7** |
...
| | **Proposed** | **138.6** | **102.5** | **74.3** | **69.3** | **76.9** | **93.0** |
...
所提去噪主导模型在保留混响的前提下去除加性噪声，相比含噪输入获得 +0.75 PESQ、+0.08 STOI、+5.21 dB SI-SDR 的提升。关键的是，去噪质量的排序与 $T_{60}$ 精度的排序（表 2）**完全一致**：去噪主导档两项任务都最优，$T_{60}$ 主导档两项任务都最差。因此辅助任务与主任务之间不存在权衡，给去噪任务更大权重能同时改善两任务，这与 §5.1 的任务权重消融结果一致。
