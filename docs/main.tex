...
error (RMSE) of \rule{1.6em}{0.4pt}\,ms, a mean absolute error (MAE) of
\rule{1.6em}{0.4pt}\,ms and a Pearson correlation of \rule{1.6em}{0.4pt}; relative
to a single-task ablation baseline ($861\,793$ parameters, RMSE
$111.15$\,ms) this is a \rule{1.6em}{0.4pt}\,\% RMSE reduction. The encoder and
...
\begin{enumerate}[(1)]
  \item We propose a multi-task framework for blind $T_{60}$ estimation
        ...
        \rule{1.6em}{0.4pt}\,\% relative to a single-task ablation baseline
        (\rule{1.6em}{0.4pt}\,ms vs.\ $111.15$\,ms), at the cost of two
  ...
  \item We carry out a systematic evaluation on four noisy reverberant
        ...
        RMSE of \rule{1.6em}{0.4pt}\,ms, a MAE of \rule{1.6em}{0.4pt}\,ms and a
        Pearson correlation of \rule{1.6em}{0.4pt}, and ablation studies
        confirm that each
...
decoders. The trade-off weights $(\alpha,\beta)$ control which task's
gradient dominates the shared backbone; we study four
configurations---$(\alpha,\beta)\in\{(1.0,0.1),(0.5,0.5),(1.0,1.0),(0.1,1.0)\}$---and
show in Section~\ref{sec:results} that the gradient magnitude ratio
...
RIR realism and to noise generalisation.
...
\begin{table}[!ht]
  \centering
  \caption{Task-weight ablation (normalised losses).}
  \label{tab:weight}
  \begin{tabular}{lccc}
    \toprule
    Configuration & $(\alpha,\beta)$ & gradient ratio $r$ & RMSE (ms) $\downarrow$ \\
    \midrule
    Denoise-dominant (proposed) & (1.0, 0.1) &  &  \\
    Balanced                    & (1.0, 1.0) &  &  \\
    $T_{60}$-dominant           & (0.1, 1.0) &  &  \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[!ht]
  \centering
  % \includegraphics[width=0.9\textwidth]{figs/gradient_cos_r.png}
  \caption{Gradient mechanism. (a) mean $\cos$ across regimes (all
  $\approx 0$); (b) mean gradient ratio $r$ (smallest for
  denoise-dominant); (c) conflict ratio neg\% (all $<50\%$); (d)
  $r$--RMSE scatter (positive correlation).}
  \label{fig:gradient}
\end{figure}

The two task gradients remain nearly orthogonal throughout training
($\cos\approx 0$, neg\% $<50\%$), with no systematic antagonistic
conflict; the decisive factor for $T_{60}$ accuracy is the gradient
magnitude ratio $r$---the smaller $r$ (the more the denoising gradient
dominates the shared backbone), the more accurate the $T_{60}$ estimate.
This indicates that pairing a complex task (denoising) with a simple
task ($T_{60}$) is best driven by letting the complex task dominate the
...
We report RMSE per $T_{60}$ bin ($0.1$\,s steps) and per SNR bin for the
proposed model and the single-task baseline.

\begin{figure}[!ht]
  \centering
  % \includegraphics[width=0.9\textwidth]{figs/per_bin.png}
  \caption{Per-bin RMSE. (a) RMSE vs.\ $T_{60}$ bin ($0.1$--$1.5$\,s,
  7 bins); (b) RMSE vs.\ SNR ($-5$ to $20$\,dB, 6 bins). Each plot:
  single-task vs.\ proposed model.}
  \label{fig:perbin}
\end{figure}
