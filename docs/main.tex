...
$T_{60}$ task rides along in fact benefits the latter.
...
representation, we apply linear probing (freeze the model, read out
$T_{60}$ by linear regression per layer, and report the
linearly-separable $T_{60}$ information as $R^{2}$) and CKA (feature
similarity between the proposed model and the single-task model per
layer).

\begin{figure}[H]
  \centering
  % \includegraphics[width=0.8\textwidth]{figs/linear_probe.png}
  \caption{Linear probing $R^{2}$ per shared layer (Encoder / TSCB$_1$ /
  TSCB$_2$): single-task vs.\ proposed model.}
  \label{fig:linearprobe}
\end{figure}

\begin{figure}[H]
  \centering
  % \includegraphics[width=0.7\textwidth]{figs/cka_heatmap.png}
  \caption{CKA similarity between each multi-task configuration and the
  single-task model, per layer (rows: Encoder $\to$ TSCB$_2$).}
  \label{fig:cka}
\end{figure}

Linear probing shows that the proposed model already builds stronger
time--frequency perception in the lower (encoder) layer, while CKA shows
that the denoise-dominant configuration reshapes the higher (TSCB$_2$)
features most thoroughly---these denoising-driven representations are
the source of the $T_{60}$ improvement.
...
The denoising quality of the proposed model on the four test sets is
reported in Table~\ref{tab:denoise}.

\begin{table}[!htbp]
  \centering
  \caption{Denoising metrics (normalised main model, average over
  test1--test4).}
  \label{tab:denoise}
  % TODO[demoise-eval]: fill Enhanced/Δ columns after running on the
  % server:  bash scripts/test_multitask_denoise_kan_norm_w_denoise.sh -g 0
  % The denoising branch is shared/identical across regimes, so the values
  % are expected to be close to the non-normalised w_denoise run
  % (PESQ 3.58, STOI 0.94, SI-SDR 21.8 dB).
  \begin{tabular}{lccc}
    \toprule
    Metric & Noisy input & Enhanced & $\Delta$ \\
    \midrule
    PESQ $\uparrow$        & 2.721 & \rule{1.4em}{0.4pt} & \rule{1.4em}{0.4pt} \\
    STOI $\uparrow$        & 0.847 & \rule{1.4em}{0.4pt} & \rule{1.4em}{0.4pt} \\
    SI-SDR (dB) $\uparrow$ & 15.91 & \rule{1.4em}{0.4pt} & \rule{1.4em}{0.4pt} \\
    \bottomrule
  \end{tabular}
\end{table}

The denoising branch removes additive noise while preserving the
reverberation, yielding clear gains in PESQ, STOI and SI-SDR.
...
under comparable settings. Representation analysis (linear probing, CKA)
further shows that denoising drives the shared layers to learn
finer-grained time--frequency discriminative features, which underlies
the $T_{60}$ improvement.
