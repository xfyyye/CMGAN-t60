...
Conformer blocks between the two tasks. The denoising task requires the
network to separate noise from the noisy reverberant spectrum and
reconstruct the
...
\begin{enumerate}[(1)]
  \item We propose a multi-task framework for blind $T_{60}$ estimation
        ...
        denoising decoder branch tapped off the shared bottleneck. To
        the best of our knowledge, this is the first work to
        systematically introduce denoising auxiliary supervision into
        blind $T_{60}$ regression with a dual-path time--frequency
        Conformer.
  ...
  \item We carry out a systematic evaluation on four noisy reverberant
        ...
        and seen/unseen noise types. Sharing the encoder and Conformer
        blocks between the two tasks, the $T_{60}$ RMSE is reduced by
        $16.4$\,\% relative to a single-task ablation baseline
        ($92.91$\,ms vs.\ $111.15$\,ms), at the cost of two denoising
        decoders ($\approx 0.54$\,M parameters; $1.41$\,M parameters in
        total vs.\ $861\,793$ for the single-task estimator). The
        proposed model attains an average RMSE of $92.91$\,ms, a MAE of
        $56.68$\,ms and a Pearson correlation of $0.9625$, and
        outperforms BERP, DAMTL and noiseaware.
\end{enumerate}
...
improves monotonically as the denoising weight is increased relative to
...
% TODO: 填入 MLP head multitask 消融结果
\begin{table}[!htbp]
  \centering
...
produced; this is consistent with the regression-head ablation of
Section~\ref{sec:res:ablation}, where the Fourier-KAN head outperforms
an MLP head of comparable capacity under the same multi-task setting.
