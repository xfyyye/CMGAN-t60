...
**数据集路径配置**: `T60_DATASET_ROOT` (23GB)

迁移到新机器时，在 `train.sh`/`test.sh` 中配置 `T60_DATASET_ROOT`，或运行时用 `-d/--dataset-root` 传入当前机器上的数据集路径。Python 脚本默认只读取 `T60_DATASET_ROOT`，也可以显式传 `--dataset_root`。
...
# 单卡
T60_DATASET_ROOT=/path/to/T60_Dataset_v7 CUDA_VISIBLE_DEVICES=1 python train.py --experiment_name CMGAN_t60_v1 --batch_size 1 --accum_steps 8
# 双卡
T60_DATASET_ROOT=/path/to/T60_Dataset_v7 CUDA_VISIBLE_DEVICES=0,1 python train.py --experiment_name CMGAN_t60_v1 --batch_size 2 --accum_steps 8

# 或用脚本
./train.sh -d /path/to/T60_Dataset_v7 -e CMGAN_t60_v1 -g 1 -b 1
```

### 测试
```bash
T60_DATASET_ROOT=/path/to/T60_Dataset_v7 python test.py --model_path runs/CMGAN_t60_v1/best_model.pth --gpu_id 0
```
